...
class Agent8(Graph_util):
    ...
    def move(self, believes_prey, believes_predator, FIRST_TIME = False):
        ...
        if FIRST_TIME:
            ...
            max_belief_predator = self.run_heuristic_predator(all_nodes, max_belief_predator)
        ...
        max_belief_prey = self.run_heuristic_prey(all_nodes, max_belief_prey)
        ...
        if not len(next_node_list):
            ...
            for i in next_node_list:
                ...
                if z>max_v:
                    ...
                    pos = i
            next_node = pos
            #next_node = self.run_heuristic_predator(next_node_list,max_belief_predator)
        self.value = next_node
        self.graph.agent = next_node
        self.path.append(next_node)
        #print("NEXT NOde : ", next_node)
    
    def run_heuristic_predator(self,next_node_list,max_belief_predator):
        '''
    ...
    def run_heuristic_prey(self,next_node_list,max_belief_prey):
        '''
        ...
        if survey:
            ...
            for i in list(self.graph.G.neighbors(max_belief_predator)):
                ...
                if len(shortest_path_from_agent_to_pred) == x:
                    ratio_60_count +=1
                    s.append(x) #Finding the count of eligible neighbors to prusue the target (60% to target)
                else:
                    ratio_40_count+=1 # Finding the count of distractions (40% to distract)
